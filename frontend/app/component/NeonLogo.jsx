...
export default function NeonLogo({ size = 'md', animated = true, showText = false }) {
  ...
  return (
    <div className={`relative flex items-center justify-center ${sizeMap[size]}`}>
      <div
        className={`absolute inset-0 rounded-full bg-green-400/20 blur-3xl ${animated ? 'animate-pulse' : ''}`}
        style={{ boxShadow: '0 0 40px rgba(34, 197, 94, 0.6), 0 0 80px rgba(34, 197, 94, 0.3)' }}
      ></div>

      <div
        className="absolute inset-0 rounded-full border-2 border-green-400/30"
        style={{ boxShadow: 'inset 0 0 20px rgba(34, 197, 94, 0.2)' }}
      ></div>

      <svg
        viewBox="0 0 200 200"
        className="relative w-full h-full"
        style={{ filter: 'drop-shadow(0 0 10px rgba(34, 197, 94, 0.8)) drop-shadow(0 0 20px rgba(34, 197, 94, 0.4))' }}
      >
        <circle
          cx="100"
          cy="100"
